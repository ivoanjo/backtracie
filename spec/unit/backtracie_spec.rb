...
RSpec.describe Backtracie do
  ...
  describe ".backtrace_locations" do
    ...
    context "when sampling a method defined in a module's singleton class" do
      module ModuleToGetASingletonMethodDefined
      end

      let(:the_singleton_class) {
        ModuleToGetASingletonMethodDefined.singleton_class.tap { |it|
          def it.test_method
            yield
          end
        }
      ...
      it do
        expect(backtracie_stack[2].qualified_method_name).to eq "ModuleToGetASingletonMethodDefined$singleton.test_method"
      end
    end
